...
const markers = [];
// 경계 목록
const bounds = new kakao.maps.Latbounds();
...
const updateMap = (infos, type) => {
  ...
  try {
    for (let i = 0; i < infos.length; i++) {
      ...
      if (type === "attraction") {
        marker.addTo(map).bindInfoWindow(setInfo(type, info.title, info.firstimage ? info.firstimage : info.firstimage2, info.tel, info.addr1, info.addr2));
        markers.push(marker);
      } else if (type === "plan") {
        marker.addTo(map).bindInfoWindow(setInfo(type, info.title, info.firstimage ? info.firstimage : info.firstimage2, info.tel, info.addr1, info.addr2));
        markers.push(marker);
...
const setInfo = (type,title,img,tel,address,addr2) => {
