const VWorldMap = {
    ...
    pinCurrentPoint: (lon, lat) => {
        ...
        VWorldMap.map.addLayer(currentPointLayer)
    },
    pinRedMarker: (lon, lat) => {
        ...
        VWorldMap.map.addLayer(markerRedLayer)
    },
    executeRealTimeLocation: () => {
        if (!navigator.geolocation) {
            console.log("실시간 위치 정보가 지원되지 않습니다.")
        }
        navigator.geolocation.watchPosition(VWorldMap.watchPositionHandler)
    },
    watchPositionHandler: (rtData) => {
        const { coords } = rtData;

        VWorldMap.removeLayer('layer_name', 'current_point_marker')
        VWorldMap.pinCurrentPoint(coords.longitude, coords.latitude)
    },
    createLayerCheckbox: (lyrKoName, lyrEnName, isLayerImg) => {
        console.dir(this);
        var html = '<div class="row" id="row' + lyrEnName + '"><label for="labelForId1"><input type="checkbox" class="layerChk" id="chk' + lyrEnName + '" value="' + lyrEnName + '" onchange="VWorldMap.layerCheckHandler(this.value)"/><span>' + lyrKoName + '</span></label></div>';
        if (isLayerImg) {
    ...
    createLayerImageCheckbox: (layerBoxName, lyrKoName, lyrEnName) => {
        var html =
          '<label for="chk' +
          lyrEnName +
          '"><input type="checkbox" class="layerChk" id="chk' +
          lyrEnName +
          '" value="' +
          lyrEnName +
          '" onchange="VWorldMap.layerCheckHandler(this.value)"/>'
    ...
    createGisMapLayer: (lyrKoName, lyrEnName) => {
        ...
        VWorldMap.map.addLayer(layer)
        VWorldMap.createLayerCheckbox(lyrKoName, lyrEnName, true)
        return layer;
    },
    createMarkerLayer: (list, lyrEnName, visible, markerImageSrc, makeTooltip, markerSize) => {

        //기존에 lyrEnName으로 그려진 layer 존재시 제거
        if (VWorldMap.getLayer(lyrEnName) !== undefined) {
            VWorldMap.map.removeLayer(VWorldMap.getLayer(lyrEnName))
        }

        var markerSource = new ol.source.Vector({
          features: VWorldMap.makeMarkerSource(
            list,
            lyrEnName,
            visible,
            markerImageSrc,
        ...
        VWorldMap.map.addLayer(markerLayer)
    ...
    createPolygonLayer: (object, lyrEnName, visible, imgClass) => {
        var polygonSource = new ol.source.Vector({
          features: VWorldMap.makePolygonSource(object, imgClass),
        })
        ...
        VWorldMap.map.addLayer(polygonLayer)
        if(typeof twLayers !== 'undefined' && !twLayers.includes(lyrEnName)) twLayers.push(lyrEnName);
    },
    createMultiLayer: (object, lyrEnName, visible, imgClass, markerImageSrc) => {
        var multiSource = new ol.source.Vector({
          features: VWorldMap.makePolygonSource(object, imgClass),
        })
        ...
        if(object.markerList.length > 0)
            polygonLayer
              .getSource()
              .addFeatures(
                VWorldMap.makeMarkerSource(object.markerList, lyrEnName, visible, markerImageSrc),
              )
    ...
    //클릭한 레이어 정보 조회
    mapClickHandler: (evt) => {
        VWorldMap.map.forEachLayerAtPixel(evt.pixel, function (layer) {
          var layername = layer.getProperties().title
          console.dir(layername)

          if (layername == 'BaseMap') return
          else if (twLayers.includes(layername)) return

          var viewResolution = VWorldMap.map.getView().getResolution()
          ...
          var query =
            bbox2.getCoordinates()[0][0].join(' ') +
            ' ' +
            bbox2.getCoordinates()[0][1].join(' ') +
            ' ' +
            bbox2.getCoordinates()[0][2].join(' ') +
            ' ' +
            bbox2.getCoordinates()[0][3].join(' ') +
            ' ' +
            bbox2.getCoordinates()[0][4].join(' ')

          var url =
            'https://gis.jeju.go.kr/arcgis/services/JJGWIMS/JJGWIMS/MapServer/WFSServer?VERSION=1.1.0&SERVICE=WFS&REQUEST=GetFeature&typename=' +
            layername
    ...
    layerCheckHandler: (target) => {
        ...
        if (target == "POI") {
            if ($("#chk" + target).is(':checked')) {
                ...
                VWorldMap.removeLayer('cluster', 'search_cluster')
                VWorldMap.removeTooltipAll('poi')
            }
            return;
        } else if (target == "MARKER") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.searchMarker()
            } else {
                VWorldMap.removeLayer('vector', 'search_vector')
            }
            return;
        } else if (target == "VIEWMAKER") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.viewMaker(data)
            } else {
                VWorldMap.removeLayer('marker', 'view_marker')
            }
            return;
            ...
            if ($("#chk" + target).is(':checked')) {
                new Promise(resolve => {
                    const checkAccidentLayer = () => {
                        if (VWorldMap.isAccidentLayerAlive) {
                          if ($('#chk' + target).is(':checked')) {
                            $('#pmAcc').show()
                          }
                          resolve()
                ...
                $("#pmAcc").hide();
                VWorldMap.removeLayer('cluster', 'search_cluster')
                VWorldMap.removeTooltipAll('tableAccPOI')
            }
            return;
            ...
            if ($("#chk" + target).is(':checked')) {
                ...
                new Promise(resolve => {
                    const checkUsageLayer = () => {
                        if (VWorldMap.isUsageLayerAlive) {
                          if ($('#chk' + target).is(':checked')) {
                            $('#pmUsage').show()
                          }
                          resolve()
                ...
                $("#pmUsage").hide();
                VWorldMap.removeLayer('allUsage', 'view_all_usage')
                VWorldMap.removeLayer('rentUsage', 'view_rent_usage')
                VWorldMap.removeLayer('returnUsage', 'view_return_usage')
                $("#vw-notice").hide();
                VWorldMap.delLayer('park')
                $("#legend").hide();
                if (VWorldMap.illegalFlag) {
                    VWorldMap.illegalFlag = false
                }
            }
            return;
            ...
            if ($("#chk" + target).is(':checked')) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        VWorldMap.timeBasedUsageResponseDto = result

                        VWorldMap.getTrackStyle(document.querySelector('.gis-range input'))

                        $('.gis-control').text("pause_circle");

                        VWorldMap.rangePlayer($('.gis-range input'))
                ...
                VWorldMap.removeLayer('circlePolygon', 'view_circle_polygon')
                $('.gis-range-wrapper').hide();
                VWorldMap.clearRangeTimer()
                $('.gis-range input').val(1); // 초기화
            }
            return;
        } else if (target == "MODLOCKHUB") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.gInfraMarker(target)
            } else {
                var GInfra = VWorldMap.gInfraType[target]

                VWorldMap.removeLayer(GInfra.source, GInfra.target)
            }
            return;
            ...
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.Bicycle)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.Bicycle.name,
                  VWorldMap.accType.Bicycle.subName,
                )
            }
            return;
        } else if (target == "ELDERACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.Elder)
            } else {
                VWorldMap.removeLayer(VWorldMap.accType.Elder.name, VWorldMap.accType.Elder.subName)
            }
            return;
        } else if (target == "CHILDRENACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.Children)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.Children.name,
                  VWorldMap.accType.Children.subName,
                )
            }
            return;
        } else if (target == "JAYWALKACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.Jaywalk)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.Jaywalk.name,
                  VWorldMap.accType.Jaywalk.subName,
                )
            }
            return;
        } else if (target == "SCHOOLZONEACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.SchoolZone)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.SchoolZone.name,
                  VWorldMap.accType.SchoolZone.subName,
                )
            }
            return;
        } else if (target == "TWOWHEELERACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.TwoWheeler)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.TwoWheeler.name,
                  VWorldMap.accType.TwoWheeler.subName,
                )
            }
            return;
        } else if (target == "LINKACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.Link)
            } else {
                VWorldMap.removeLayer(VWorldMap.accType.Link.name, VWorldMap.accType.Link.subName)
            }
            return;
        } else if (target == "ROADIDXACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.lineStringMaker()
            } else {
                VWorldMap.removeLayer('roadIdx', 'view_roadIdx')
            }
            return;
        } else if (target == "WALKERACC") {
            if ($("#chk" + target).is(':checked')) {
                VWorldMap.polygonMaker(VWorldMap.accType.WalkerAcc)
            } else {
                VWorldMap.removeLayer(
                  VWorldMap.accType.WalkerAcc.name,
                  VWorldMap.accType.WalkerAcc.subName,
                )
            }
            return;
        ...
        var selectLayer = VWorldMap.getLayer(target)
        selectLayer.setVisible($("#chk" + target).is(':checked'));

        if ($("#chk" + target).is(':checked')) {
            if (target == "HybridMap") {
                VWorldMap.getLayer('SatelliteMap').setVisible(true)
            }
        } else {
            if (target == "HybridMap") {
                VWorldMap.getLayer('SatelliteMap').setVisible($('#chkSatelliteMap').is(':checked'))
            } else if (target == "SatelliteMap") {
                VWorldMap.getLayer('SatelliteMap').setVisible($('#chkHybridMap').is(':checked'))
            }
        }
    },
    controlLayerHandler: (layerName) => {
        if (layerName == "HybridMap") {
            VWorldMap.getLayer('SatelliteMap').setVisible(true)
            VWorldMap.getLayer('HybridMap').setVisible(true)
        } else if (layerName == "SatelliteMap") {
            VWorldMap.getLayer('SatelliteMap').setVisible(true)
            VWorldMap.getLayer('HybridMap').setVisible(false)
        } else if (layerName == "2d-map") {
            VWorldMap.getLayer('HybridMap').setVisible(false)
            VWorldMap.getLayer('SatelliteMap').setVisible(false)
        }
    },
    ...
    delLayer: (layerName) => {
        var layer = VWorldMap.getLayer(layerName)
        VWorldMap.map.removeLayer(layer)
    },
    removeLayer: (source, target) => {
        var array = [];

        VWorldMap.map.getLayers().forEach(function (layer) {
        ...
        while (array.length) {
            ...
            VWorldMap.map.removeLayer(lastElement)
        }
    },
    layerDivControl: () => {
    ...
    goPoint: (x, y, layer) => {
        if (x != null && x != "" && y != null && y != "") {
            ...
            VWorldMap.map.getView().setCenter(coordTransAddr)
            VWorldMap.map.getView().setZoom(parseInt(16))
            $("#chk" + layer).prop("checked", true);
            VWorldMap.layerCheckHandler(layer)
        }
    },
    searchAddress: (x, y) => {
        if (x != null && x != "" && y != null && y != "") {
        ...
        VWorldMap.map.addInteraction(addressActivation)
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                */
                ...
                VWorldMap.map.addLayer(clusters)
            },
    ...
    move: (x, y) => {
        VWorldMap.map.getView().setCenter(ol.proj.transform([x, y], 'EPSG:4326', 'EPSG:900913')) // 지도 이동
        VWorldMap.map.getView().setZoom(12)
    },
    searchMarker: () => {
        var success = (data) => {
            var features = new Array();
...
*/
            ...
            */
            ...
            VWorldMap.map.addLayer(vectorLayer)
        };
    ...
    polygonMaker: function(TypeClass) {
        ...
        if (TypeClass.code === "07") {
            $.ajax({
                ...
                success: function (res) {
                    (async () => {
                        await VWorldMap.showGisPopup(res.length)

                        res.forEach(function (element) {
                            var coordinateList = VWorldMap.wktToGeoJSON(
                              element.acdntRiskRng,
                            ).coordinates
                            VWorldMap.drawPolygon(
                              coordinateList,
                              'polygonLayer',
                              true,
                              'polygon1',
    ...
    setCenterCustom: function (xy, zoom) {
        ...
        VWorldMap.map.getView().setCenter(coordTransAddr)
        VWorldMap.map.getView().setZoom(zoom)
    },
    ...
    refreshMap: function (xy, zoomLevel) {
        ...
        VWorldMap.map.getView().setCenter(coordTransAddr)
        VWorldMap.map.getView().setZoom(zoomLevel)
    },
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data.forEach(function(datum, index) {
                    ...
                    VWorldMap.removeLayer('illegalParking', 'search_illegal_parking')

                    VWorldMap.map.addLayer(illegalParkingLayer)
                });
            }
        });
    },
    ...
    gInfraMarker: function(gInfraType) {
        var GInfra = VWorldMap.gInfraType[gInfraType]
        ...
        $.ajax({
            ...
            success: function (list) {
                (async () => {
                    await VWorldMap.showGisPopup(list.length)
                    ...
                    VWorldMap.map.addLayer(gInfraLayer)
                })();
            }, error: function (error) {
                alert('정보 조회에 실패했습니다.\n해당 문제가 지속될 경우 관리자에게 문의하여 주십시요.');
                console.log(error.code);
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        (async () => {
                            ...
                            VWorldMap.map.addLayer(lineStringLayer)
                        })();
                    },
    ...
    getTrackStyle: function (el) {
        ...
        if (VWorldMap.timeBasedUsageResponseDto) {
          for (var key in VWorldMap.timeBasedUsageResponseDto.rentTimeMap) {
            if (parseInt(key) + 1 === parseInt(el.value)) {
              var list = VWorldMap.timeBasedUsageResponseDto.rentTimeMap[key]

              list.forEach(function (object) {
                VWorldMap.drawCirclePolygon(object, 'blue')
              })
              break
            }
          }
    ...
    rangePlayer: function (rangeInput) { // 시간대별 이용 현황 range 자동 재생시 사용되는 함수
            var timerId = setTimeout(() => {
                ...
                VWorldMap.timers.splice(0)

                VWorldMap.rangePlayer(rangeInput)
            }, 3000);

            VWorldMap.timers.push(timerId)
    },
    clearRangeTimer: function() {
        for (var i = 0; i < VWorldMap.timers.length; i++) {
          clearTimeout(VWorldMap.timers[i])
        }
    },
