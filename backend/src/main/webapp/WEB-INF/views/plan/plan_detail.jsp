...
<body>
			...
			<div id="mapOverlayList">
				<c:forEach var="item" items="${list}">
					<div class="mapOverlayItem">
						<button class="red-btn" data-id="${item.ppid}" onclick="onClickRedBtn(event, ${item.ppid})">X</button>
						${item.attraction.title}
					</div>
				</c:forEach>
			</div>
			<button class="btn btn-primary w-100 mb-3" onclick="location.href='/plans/timeline?name=${plan.name}'">계획 보기</button>
		</div>
	</div>
	<%@ include file="../fragments/footer.jsp" %>

	<!--  카카오맵 API  -->
	<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=b62a7b977e891121b298d190876cda4a&libraries=services,clusterer"></script>
	...
	<script>
		...
		document.getElementById("searchBtn").addEventListener("click", function (event) {
			...
			fetch(`/attractions/keyword` + `?keyword=` + keyword)
					.then(response => {
						if (!response.ok) {
					...
					.then(data => {
						...
						mappedData.push({ ...dt, latlng: { x: dt.mapx, y: dt.mapy } });
						KakaoMap.updateMap(mappedData, "plan", ${plan.pid});
					})
					.catch(error => {
						console.error("데이터 불러오기 실패:", error);
